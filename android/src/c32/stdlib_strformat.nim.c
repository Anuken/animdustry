...
{nimErr_ = nimErrorFlag();
	...
	LA30_: ;
	parsedLength = parseSaturatedNatural__pureZparseutils_316(s, (&result.minimumWidth), i);
	if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
	i += parsedLength;
	{
		NIM_BOOL T34_;
		...
		if (!T34_) goto LA36_;
		i += ((NI) 1);
		parsedLengthB = parseSaturatedNatural__pureZparseutils_316(s, (&result.precision), i);
		if (NIM_UNLIKELY(*nimErr_)) goto BeforeRet_;
		i += parsedLengthB;
	}
	goto LA32_;
		...
		if (!(0.0 <= value)) goto LA18_;
		{
			...
			sign = NIM_TRUE;
			{
				if (!(value == 0.0)) goto LA26_;
				{
					...
					insert__system_6314((&f), colontmpD__4, ((NI) 0));
					eqdestroy___system_2592((&colontmpD__4));
				}
				LA30_: ;
			}
			...
			LA26_: ;
			{
				...
				insert__system_6314((&f), colontmpD__5, ((NI) 0));
				eqdestroy___system_2592((&colontmpD__5));
			}
			LA24_: ;
		}
